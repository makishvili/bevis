...
Вы же не в notepad код пишете, надеюсь?
Вы пользуетесь аутокомплитом, то есть написали открывающую скобку, затем букву `d` и нажали хоткей для аутокомплита.
Или если Дзен-кодингом набираете html, который придумал Вадим Макеев, а разработал Сергей
Чикуёнок (и теперь он называется Emmet), тогда вы вообще пишете три символа div и нажимаете tab.

Ну, а мы пользуемся сниппетами. Нажал хоткей, вписал только три символа `div` и у нас готова строка ```javascript ctx .setTag ('div'); ```
