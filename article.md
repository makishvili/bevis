...
который `header.styl` генерит CSS-селекторы, а потом уже их копирует в `/pages/index/index.css`.

8. Дальше `ENB`  dыходит из `/blocks/header` и заходит в `/blocks/footer` и делает то же самое.
...
есть `ENB` занимается тем, что создаёт страничные файлы.
