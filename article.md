...
3. Описание страницы обязано быть простым
  ...
  * Блок - это паттерны Модуль и Фасад.
  * Триединство структура-представление-поведение
  * Изоляция, безопасность, расширяемость.
4. Модуль и Фасад в HTML
...
5. Зачем нужны декларативные шаблоны?
  * Реюзабельность шаблонов (но инклюды императинвых шаблонов позволяют тоже)
  * Блок - это json с фиксированным набором опций. Совсем, как js-функция. Это Фасад.
  * Теневое дерево. Совсем как в Web Components. Это Модуль.
  * Работы столько же, бонусы круче  --> НЕ ПОЛУЧАЕТСЯ ЗАПИСАТЬ В ЭТОЙ ГЛАВЕ ПРОФИТ. НЕ НАХОЖУ СЛОВ ПОДХОДЯЩИХ! НЕ ПОЛУЧАЕТСЯ ЧЁТКО!
6. Паттерн "Модуль" в CSS
Надо понять, как описать эти задачи:
  * Управляемая последовательность стилей вместо миксования блоков на одной ноде
  * Абсолютная независимость стилей блоков друг от друга (паттерн модуль, он же как аналог изоляции стилей внутри одного WebComponents)
7. Паттерны "Фасад" и "Модуль" в клиентском Javascript
...
Это важно. Описание страницы должно быть максимально простым.
Чем предмет проще, тем легче им пользоваться. Бабушка
...
Мы хотим получать удовольствие от верстки, а не боль. Описывать страницу тегами  - это сложно — слишком уж много тегов,
атрибутов, мало семантики и смысла.

Вот простое описание блоков.
```javascript
[
    {
        block: 'header'
...
Смотрите, здесь не описано ни одного элемента, ни одного тега и атрибута. Но из каждой декларации Бивис сгенерит
...
Предположим, на нашей странице будет только шапка
...
Как нужно написать шаблон, чтобы json превратился в примерно такой html?
```html
<div class="header"></div>
```

Нужно где-нибудь написать такое:
```javascript
...
Но такой шаблон выглядит довольно бесполезно. Давайте усложнять. Я хочу, чтобы внутри шапки появился заголовок.
...
Но это всё синтеттические примеры. Посмотрим на реальный пример их Яндекса. Это описание шапки.
...
## Паттерны "Фасад" и "Модуль" в HTML
...
Но мы в бивисе получаем верстку, которая выполняет трбования известных паттернов программирования: Модуль и Фасад. И всё это внутри html-верстки.

## Модуль

Блок - это такая же абстракция, как то, что в Javascript вы знаете под паттерном Модуль
...
Эти же слова применимы и к нашей абстракции.
Web-блок, как и набирающие сейчас популярность web-component, состоит из трёх частей: структура, внешний вид и поведение. Чтобы было понятнее,
представьте меня в виде модуля, в виде блока:
...
И ещё у меня и у блока общее - это имя.

У меня имя одно - Вадим. У Марата тоже одно. И у блока имя должно быть одно: хедер, ауторизейшн, футер.
Одного имени _должно быть_ достаточно, чтобы блоку построить и html, и чтобы приодеть его в стии, и чтобы научить блок вести себя на странице.

Бивис постулирует - одного имени достаточно.

Оно используется и bt-шаблонах, и в качестве css-класса html-узла (что позволяет нам потом стилизовать html-узел),
оно же копируется в специальный data-атрибут, который указывает блоку, куда смотреть, чтобы узнать, как именно ты взаимодействуешь с пользователем.

Как оно используется в bt, мы уже знаем, а вот как  оно же испольуется для стилей и поведения:

```html
<div class="header"        <!-- Здесь испольузется для стилизации -->
     data-block="header">  <!-- здесь для задания поведения на странице -->
</div>
```

Имя может быть простым — просто Марат, или просто Вадим, или просто header. Но бывают случаи, когда оно должно быть более точным.
...
Понимаете, блок один - шапка, но она может быть поисковая и не поисковая. Поэтому у неё есть два варианта отображения на странице,
два представления, две темы, два скина, две шкурки... два `View`. Разный вид одного и того же блока мы называем разным преставлением. Разным `View`.

Есть ещё одна абстракция, о кторой важно сказать.
...
Итого, в `BEViS` всё однозначно. Блок может иметь только одно имя.

Это и есть основа паттерна "Модуль" в `CSS`


## Паттерн "Модуль" в CSS

При этом мы не простачки какие-то, чтобы отказываться от чудесного механизма множественных селекторов. Мы просто используем их на один шаг раньше - на сервере.

Только я один (разработчик проекта) знаю как должен выглядеть блок в той или иной ситуации. Это я должен сообщать браузеру, а не он, встречая
на одной ноде несколько классов. Приоритет можно повысить тремя способами:

1. порядок селекторов в файле

2. повышения специфичности имени селектора (добавление id или дополнительных родительских классов)

3. дать свойству флаг `!important`

Флаг `!important` мы не ставим, это путь в хаос и боль.
Повышать специфичность мы тоже себе запрещаем, потому что наш постулат о простоте гласит: одного имени блока должно быть достатночно для всего.
Остаётся порядок селекторов в файле.

Этим порядком управляем мы на сервере с помощью CSS-препроцессора. У врачей есть специальный термин — "под контролем глаза".
...
Содержимое главы:
* Управляемая последовательность стилей вместо множественных классов (точнее, вместо миксования блоков на одной ноде)
* Абсолютная независимость стилей блоков друг от друга (паттерн модуль, он же как аналог изоляции стилей внутри одного WebComponents)
* Совсем полная будет когда мы изолируем каскадныей стили на html-узлы (или кастомные теги, о ктороых браузер не знает, или обнуление дефолтных браузерных стилей для каждого блока. Это что - идея
майерского ресета, раскопированного в каждый блок?)

Решаем все задачи с помощью
* Stylus (через единственный css-класс блока)
...
Стили мы пишем на Stylus, потому что препроцессор позволяет сделать стили блоков не только действительно независимыми друг от друга, но ещё и удобными в поддержке.

Возьмём частую задачу в верстке. БлокДва блока имеют одинаковые

Например, еслть группа общих свойств для всех блоков - гарнитура шрифта, размер шрифта, цвет шрифта.
Путь первый - олдскульный:
...
.footer {
    /* свойства для подвала */
}
````

Но это неудобно. Создавая новый блок, нужно не забыть поправить первый селектор, потом убедиться, что он попадёт на страницу действительно раньше остальных, чтобы не перетёр стили самого блока.

Это по сути та же реализация, только с использованием множественных классов:
````css
.common {
    /* общее */
}
...
Страдает теми же недостатками, что и олдскульный.

Чтобы не гадать, какие селекторы придут после каких, нужно исключить ситуацию, когда мы НЕ контролируем последовательность селекторов, применяемых к блоку. Сделать это можно только явно управляяю
этим руками на этапе формирования стилей. Например, так:
...
Этот Stylus код превратится в такой css
...
Мы точно получаем правильную последовательсноть стилей внутри одного класса.

----

Итак, Модуль - это когда все сложности спрятаны за простым именем. Одного имени хватает для триединства. И в этом секрет простоты Бивиса.
Фасад - это когда у модуля есть зафиксировнное АПИ в виде опций/параметров блока.

----


## Паттерны "Фасад" и "Модуль" в клиентском Javascript
