...
2. Абстракция над DOM
3. Описание страницы обязано быть простым
4. Декларативные шаблоны
5. Паттерны "Фасад" и "Модуль" в HTML
...
Содержимое главы:
* Проблема весов селекторов для одного блока. Она есть. Её нужно решать (где-то в тексте я про неё начинал, найди)
...
Для чего нужен CSS-класс? По уму, только для стилизации, чтобы приодеть голые html-теги, чтобы они своими угловыми скобками не царапали пользователям глаза.
Другие фреймворки используют css-класс ещё и для того, чтобы javascript-писать для блока. Взять тот же jQuery с его $('.selector'), который нужен,
чтобы получуить DOM-ноду. Мы так не делаем, но об этом чуть позже. А сейчас вернемся к стилям
