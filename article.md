...
##Зачем такое надо?
Мы это сделали не по самой популярной среди гиков причине. Кстати, знаете, какая самая частая причина появления новых велосипедов?
Она описывается одним предложением: "Because I can".

Сделать такой шаблонизатор нас вынудили две внешние и одна внутренняя причины.

Внутренняя - мы любим простоту. Именно поэтому мы сделали такой лаконичный формат описания страницы.

И две внешних:
1. нам нужна реюзабельность кода. Мы не хотим html-код блоков копировать из страницы в страницу.

И тут было из чего выбирать среди готовых решений. Но ни одно из существующих решений на javascript не удовлетворяет второму условию:

2. реализация блока должна быть отделена от использования. В большом программировании это называется паттерн "интерфейс", когда класс выставляет наружу интерфейсные
методы, которые не меняются и поддерживаются пожизненно, а внутрення реализация класса скрыта от глаз, находится в приватной области. В верстке мы хотели
ровно этого же, потому что только такой способ гарантирует стабильность кода при реюзабельности.

BTJson блока - это внешний интерфейс, а BT-шаблоны - это приватная реализация блоков.

Ощутите мощь этой идеи на примере: при простой декларации шапки
...
Верстка может какой угодно сложной. Но не это самое ценное. Сила в том, что вы отныне можете менять шаблоны, генерирующие html как угодно часто,
вам не нужно при этом переписывать все старницы, все объявления блока. Потмоу что блок имеет строгое АПИ, опсианное  в виде параметров блока.
