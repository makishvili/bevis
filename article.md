...
1. В адресную строку в браузере пользователь вводит `http://my-server/index.html`.

2. Браузер отправляет http-запрос к серверу и ждёт страницу `index.html`

3. Node.js-сервер принимает запрос. Понимает, что нужно создать
страницу `index.html` и поэтому он открывает на чтение файл `/pages/index/index.page.js`

4. После этого он парсит `index.btjson.js` (анализирует каждый элемент btjson-массива и запоминает из каких блоков будет построена будущая `HTML`-страница)
...
7. Ищет файл `header.css`. Если он есть - копирует его содержимое в `/pages/index/index.css`. Делает то же самое для
`js` и `bt`-файлов, создавая `/pages/index/index.js` и `/pages/index/index.bt.js`.
...
сгенерённый `index.bt.js`. Напомню,
