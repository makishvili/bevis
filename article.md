...
5. CSS блока
  * Нельзя миксовать блоки на одной ноде
    ...
    * частые юзкейсы для смешивания
    * как эти юзкейсы решает бивис
    * это копипаст?
    * это осознанная стабильность, "под контролем глаза"
  * Мы делаем это на препроцессоре
    * те же примеры показать на препроцессоре
    * вернуться к view - как оно помогает решить верхний юзкейс
    * коснуться state
...
```
Мы делаем разные преставления одного блока, а каждое из них содержит в себе стили базового представления
...
Представьте, сколько проблем сулит уже тройное примешивание:
```html
<div class="menu menu-top clearfix"></div>
```

Даже небольшое изменение стилей для клирфикса нужно согласовывать со стилями меню и меню-топ.

А ведь это не синтетический пример. Это именно то, чем нам не подходит БЭМ-подход, в котором вот так можно записать:
```
<div class="button button_theme_normal button_shadow_yes button_hovered_yes"></div>
```
Это вполне реальный пример кнопки, у которой 4 класса.
Каждый класс с любой момент времени может сломать внешний вид кнопки.

В Бивисе ничего не происзодит без нашего ведома. Только явно, только открыто.

Мы делаем это с помощью CSS-препроцессора.
...
Препроцессор - это тот же css, только на сервере собирается в css из других файлов
Описать другими словами, чтобы понятно было из одного предложения.


И помогают нам в этом `view`.
Когда у нас есть кнопка, которая может иметь нормлаьную цветовую тему и тень под собой, мы
создаём отдельное предствление, отдельное view, которое называем как угодно красиво:
можно button_normal-shadow, можно button_awesome, можно button_makishvili.
Лишь бы разработчики понимаи, что кроется за этим вью.

А уже внутри вью мы сами определяем в каком плрядке расположить селекторы из модификатора темы,
из модификатора тени так, чтобы они не конфликтовали. Мы руками разруливаем все конфликты, и сохраняем эти
стабильные взаимоотношения селекторв внутри специального класса view.

Вот почему мы можем сделать так
```
<div class="menu_main"></div>
<div class="menu_top"></div>
<div class="menu_bottom"></div>
