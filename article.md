...
skin-common(), skin-theme-normal(), skin-shadow() - это так называемые миксины, примеси по-русски.
Это те же javascript-функции, которые можно вызвать, а они на место выова вставят своё содержимое.
Та же идея, что у инклюдов.

Идея в том, сто эти функции приватны. Они не попадают напрямую в скомпилироанный css-файл. Пока я их конкретного селектора не выову этот миксин, я его не получу
А я и зову их явно внутри каждого селектора. Создал селектор для нормальной кнопки с тенью и позвал в него нужные миксины, которые тут же внутри селектора разрулил
