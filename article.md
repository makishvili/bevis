...
_Бивис — еще один подход в разработке сайтов, который придумали в Яндексе.
Это простая и понятная модель разработки, которая подходит для больших проектов, повышает их надежность._

_Это будет дружеский рассказ, такой можно услышать в офисных коридорах или на кофепойнте.
Мы расскажем, как верстаем свои проекты, и почему отныне уверены в их стабильности. Вдруг и вам Бивис
приглянётся :)_
...
Привет, я Вадим, это мой друг Марат. Мы из Яндекс.Карт.
...
1. Запрос из браузера попадает на какой-то http-сервер — Apache или Nginx, не суть важно.

2. Дальше запрос направляется в контроллер - программу, написанную, например, на PHP или на Python.
У контроллера простые задачи — получить данные из бекенда (или напрямую из базы данных) и передать в шаблоны представления

3. Шаблоны сегенерят html и ответ уйдёт назад в браузер.

Вам знакома эта схема? Кажждому знакома эта схема. Ну так мы её и не меняли.
