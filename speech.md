...
Да и сами блоки создавать тожде просто, потому что это очень жизненная абстракция.
...
```
Мы матчимся по селектору, для которого выполянется функция.
...
#### Это оверхед?
**Вадим:** Марат, это не кажется тебе оверхедом? Императивные шаблоны Смарти и Джанго привычные, понятные, их понимают все.
Почему мы не используем их?

**Марат:** Императивные шаблоны остаются понятными, пока их немного. Когда проект разростается, все больше и больше визуальных фрагментов переиспользуются, то и шаблоны становятся сложными и запутанными. В такой ситуации BT-шаблоны позволяют сохранять сложность на одном уровне.

Более того, BT-шаблоны позволяют нам иметь явный и понятный API каждого фрагмента страницы: блока. BTJson блока - это внешнее АПИ блока, а BT-шаблоны - это приватная реализация блоков.
...
я их здесь же (всё ещё на сервере) разрузлил, и в браузер пойдёт css-селетор, в котором полный порядок. Мы можем решить конфликты на этапе разработки, а не на этапе использования
...
Третья - имена модулей-зависимостей. Пока они не скажут про себя, что они готовы, модуль form не начнёт ничего делать.
...
**Марат:** Во-первых, мы получаем код, который можно повторно использовать. А во-вторых — абстракцию над HTML-структурой блока. Мы переходим от одной сущности — DOM-элементы, к другой — блоки с их семантикой.
...
**Марат:** Сборка часто являлась узким местом в сложных компонентных системах, но в рамках BEViS мы полностью интегрировали сборку в процесс разработки. Работая над проектом достаточно лишь обновлять страницу в браузере, чтобы всегда видеть актуальные стили, скрипты и верстку. 
...
**Марат:** Кроме того, мы реализовали страницу в стиле TodoMVC на Бивисе: http://github.com/bevis-ui/bevis-todo. Можно сравнить Бивис с другими MVC и компонентными фреймворками.

**Вадим:** Пользоваться бивисом одно удовольствие, потому что 99% бивиса - это валидный javascript.
