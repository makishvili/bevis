...
Для начала мы добавляем элемент title, который будет заглоdком нашего блока.
...
Когда проект разростается, все больше и больше визуальных фрагментов переиспользуются,
...
И мы можем не бояться, что что-то пойдёт не так. Потому что именно соблюдение АПИ как и
 гарантируют нам результат.
