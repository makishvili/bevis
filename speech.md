...
**В:** В чём секрет твоих продуктов, Марат?

**М:** Спасибо, Вадим. Здравствуйте. Я считаю, что главное в программном продукте - это простота и понятность. Когда мы начинали придумывать Бивис,
...
**В:** Спасибо, Марат. Да, мы должны сразу признаться вам. У нас нет никаких целей. Мы пришлю сюда просто, чтобы поговорить с вами.
...
**В:** Марат, расскажи мне, как устроено обычное MVC-приложение. Я когда писал на PHP, на Смарти. Я что-то помню.

**М:** Ок. Обычное MVC-приложение работает по стандартной схеме:
...
**В:** Да, мне знакома эта схема.

**М:** Каждому знакома эта схема. Ну так мы её и не меняли.

**В:** Бивис работает по этой же схеме?

**М:** Да, всё то же самое, только...
...
**В:** Я так понимаю, вы сделали ещё один серверный велосипед, только на яваскрипте? Зачем? Ведь есть иСмарти, и Джанга, и XSLT. Сейчас и
клиентские всякие есть - Ангуляр тот же. Зачем ещё один?

**М:** Нет, велосипед мы взяли старый, только заменили в нём устаревшие детальки. У нас две причины:
...
**В:** Мне нравится мысль, не учить серверные языки. Хотя, PHP я уже немного знаю. В PHP или в том же Python контроллер - это
набор PHP/Python-команд, которые ходят в бекенд за данными. А как выглядит ваш контроллер `index.page.js?`? Что внутри?

**М:** Это набор `Javascript`-команд. Так как мы пишем на `Node.js`, наш яваскрипт завернут в `Node.js` модули.
...
**В:** первая строка - это что такое?

**М:** Если вы на `Node.js` не писали, не пугайтесь. Не обращайте внимания на первую строку.
...
**В:** `JSON`? Почему `JSON`? Почему нет `HTML`-тегов? Но я и сам знаю ответ на этот вопрос.
